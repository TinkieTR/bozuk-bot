...
if(!args[0]) return message.channel.send("```- Sistemi kullanmak için, -uyarı ekle/sil/bilgi komutlarını kullanın.``` \n\n```-uyarı ekle @Kişi (Sebep)```\n<a:mavikalp:797222086895599636> **Belirttiğiniz Kişiye Uyarı Ekler. ⤴**\n\n```-uyarı sil @Kişi (Adet)```\n<a:mavikalp:797222086895599636> **Belirttiğiniz Kişinin Uyarısını Siler. ⤴**\n\n```-uyarı bilgi (@Kişi)```\n<a:mavikalp:797222086895599636> **Belirttiğiniz Kişinin Uyarısını Siler. ⤴** ")
...
await kullanıcı.send(`${kullanıcı}, Merhaba Ben **${message.guild.name}** Sunucusunda Bir Botum Ve Beni **Seni Uyarmam** İçin Gönderdiler.\n\n <a:mavikalp:797222086895599636> Neden Uyarıldığını Bilmiyorum Ama Sen Yinede Dikkatli ol !`) 
return}

if(reason) {
await message.channel.send(`${kullanıcı}, uyarıldı!\nToplam uyarı sayısı: ${syı}`)
await kullanıcı.send(`${kullanıcı}, Merhaba Ben **${message.guild.name}** Sunucusunda Bir Botum Ve Beni **Seni Uyarmam** İçin Gönderdiler.\n\n <a:mavikalp:797222086895599636> **${reason}** Sebebiyle Uyarıldın. Dikkatli ol ! `) 
return} }
